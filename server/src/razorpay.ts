...
import { env, hmacSha256, HttpError, log, safeEqualHex } from "./core";
...
export async function cancelSubscription(id: string): Promise<void> {
  await rp("POST", `/subscriptions/${id}/cancel`, { cancel_at_cycle_end: 1 });
}

/** Hosted-checkout response: razorpay_signature = HMAC_SHA256(payment_id|subscription_id, secret). */
...
export function verifyWebhookSignature(rawBody: string, signature: string | undefined): boolean {
  ...
  const expected = hmacSha256(env.razorpayWebhookSecret, rawBody);
  try {
